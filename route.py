...
class Route:
    ...
    def __init__(self, route=None):
        ...
        if route == None:
            for i in range(RouteManager.numberOfNodes()-1):
                self.base.append(Node(-1, -1))
    ...
    def generateIndividual(self):
        ...
        for dindex in range(1, RouteManager.numberOfNodes()):
    ...
    # Returns j'th node in i'th route
    def getNode(self, i, j):
        return self.route[i][j]

    # Sets value of j'th node in i'th route
    def setNode(self, i, j, db):
        self.route[i][j] = db
            ...
            for i in range(numTrucks):
                for j in range(self.routeLengths[i]):
                    ...
                    if j+1 < self.routeLengths[i]:
                        destinationNode = self.getNode(i, j + 1)

                    else:
                        destinationNode = self.getNode(i, 0)
    ...
    # Checks if the route contains a particular node
    def containsNode(self, db):
